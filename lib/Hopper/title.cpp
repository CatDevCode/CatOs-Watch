...
/*---------------------------------------------------------------------------*/
static uint8_t eeprom_emu[1024];
static bool eeprom_loaded = false;

static void loadEeprom() {
    if (eeprom_loaded) return;
    File f = LittleFS.open("/hopper.dat", "r");
    if (f) {
        f.read(eeprom_emu, sizeof(eeprom_emu));
        f.close();
    } else {
        memset(eeprom_emu, 0, sizeof(eeprom_emu));
    }
    eeprom_loaded = true;
}

static void saveEeprom() {
    File f = LittleFS.open("/hopper.dat", "w");
    if (f) {
        f.write(eeprom_emu, sizeof(eeprom_emu));
        f.close();
    }
}
...
void eepSeek(int addr) {
    eepAddr = addr;
    if (!eeprom_loaded) loadEeprom();
}

uint8_t eepRead8(void) {
    if (!eeprom_loaded) loadEeprom();
    if (eepAddr < 0 || eepAddr >= 1024) return 0;
    return eeprom_emu[eepAddr++];
}
...
void eepReadBlock(void *p, size_t n) {
    if (!eeprom_loaded) loadEeprom();
    if (eepAddr < 0 || eepAddr + (int)n > 1024) return;
    memcpy(p, &eeprom_emu[eepAddr], n);
    eepAddr += n;
}

void eepWrite8(uint8_t val) {
    if (!eeprom_loaded) loadEeprom();
    if (eepAddr >= 0 && eepAddr < 1024) {
        eeprom_emu[eepAddr++] = val;
        saveEeprom();
    }
}
