...
#define mid(a, b, c)    max((int)min((int)(b), (int)(c)), (int)(a))
...
static void fillPatternedRect(int16_t x, int16_t y, uint8_t w, int8_t h, const uint8_t *ptn)
{
    ...
    int16_t x0 = (x < 0) ? 0 : x;
    int16_t y0 = (y < 0) ? 0 : y;
    int16_t x1 = x + w; if (x1 > HOPPER_WIDTH) x1 = HOPPER_WIDTH;
    int16_t y1 = y + h; if (y1 > HOPPER_HEIGHT) y1 = HOPPER_HEIGHT;
    if (x0 >= x1 || y0 >= y1) return;

    for (int16_t py = y0; py < y1; py++) {
        for (int16_t px = x0; px < x1; px++) {
            uint8_t patByte = ptn[px & 3];
            if (patByte & (1 << (py & 7))) {
                oled.dot(px, py, 1);
            }
        }
